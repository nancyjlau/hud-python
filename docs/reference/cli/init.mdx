...
Start the development server. Add `--watch` (`-w`) to enable hot-reload:
```bash
...
# Or connect a GitHub repo on hud.ai → New → Environment
```
</Step>
</Steps>
...
- [hud dev](/reference/cli/dev) – Development server (`--watch` for hot-reload)

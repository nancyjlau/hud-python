---
title: "hud dev"
description: "Development server for MCP environments (hot-reload is opt-in via --watch)"
icon: "fire"
---

The `hud dev` command provides development-time proxying for MCP environments, starting your image and exposing it over HTTP (default) or stdio. Hot-reload is opt-in with `--watch`/`-w`.
...
- **Opt-in hot-reload**: Use `--watch`/`-w` to enable file watching
...
| `--watch`, `-w` | Paths to watch for hot-reload (repeatable) | none |
...
By default this runs without file watching.
...
4. **Hot-Reload**:
   - Disabled by default
   - Enabled only when `--watch`/`-w` is provided
   - Watches the specified files/folders and restarts the controller on change
...
2. Add to Cursor's MCP config:
   ...
   ```
3. Edit files in watched paths (`-w`) and the controller restarts automatically.
...
- [Deploy](/quick-links/deploy) — Push to GitHub and deploy
